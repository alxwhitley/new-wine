...
│       ├── admin/          # Admin modal + corpus panel (AdminModal.tsx, CorpusDocumentsPanel.tsx)
│       └── ui/
...
│   ├── run_queue_triage.py    # Stage 5 Run 1: Queue-driven triage orchestrator (reads Queue tab, drives process_sheet per source; --time-limit MINUTES)
│   ├── run_queue_ingest.py    # Stage 5 Run 2: Queue-driven ingest orchestrator (walks source tabs, drives ingest_sheet per tab; --time-limit MINUTES)
...
- `original_title` (text, nullable) — title as captured at ingest before any display normalization; backfilled on 3,678 rows via migration 053; shown in CorpusDocumentsPanel when it differs from `title`
...
**`removed_urls` table** — blocklist of intentionally-deleted documents (migration 054, APPLIED 2026-06-29)
- `url` (text PRIMARY KEY), `document_id` (uuid), `title` (text), `original_title` (text), `removed_at` (timestamptz NOT NULL DEFAULT now()), `reason` (text)
- Written by `DELETE /admin/document/{id}` before the delete so concurrent re-ingest attempts see the block record. Checked by `youtube_ingest.py` before each ingest call (non-fatal skip on hit). No RLS defined — service-role access only.
...
- **Corpus v2 admin UI (June 2026):** AdminModal popup restructured to 4 top-level tabs: Corpus, Feedback, Contributors, Notes Queue. Corpus has 3 sub-views: Documents (paginated via `GET /admin/corpus/documents` — server-side filters, stats bar, View Sheet, Remove AlertDialog; `DELETE /admin/document/{id}` writes `removed_urls` before delete), Sources (read-only license badge, visibility Switch, manage Sheet with CopyButton for source ID), Pipelines (stats pills + command reference with CopyButton per command). Governance and Pipelines are no longer top-level tabs. New components: `frontend/components/ui/alert-dialog.tsx` (Radix UI AlertDialog primitive), `frontend/components/admin/CorpusDocumentsPanel.tsx`. Realtime uses unique channel name per mount (`admin-realtime-${Date.now()}`) — prevents "cannot add postgres_changes after subscribe()" crash on re-open; failure wrapped in try/catch (graceful degrade). `GET /admin/stats` service-key bypasses RLS. `GET /admin/license-sources` bulk fetch + Python Counter.
...
- ~~**Admin panel crashes on open ("This page couldn't load")**~~ — **FIXED (June 2026):** Two root causes, both fixed: (1) Missing `Query` import in `backend/app/routers/admin.py` caused `NameError` at module import time → uvicorn startup crash → all `/admin/*` routes absent (commit ebf2f04). (2) Supabase realtime: re-opening the modal called `.on()` on an already-subscribed `"admin-realtime"` channel → "cannot add postgres_changes after subscribe()" crash. Fixed by using a unique channel name per mount (`admin-realtime-${Date.now()}`), wrapped in try/catch for graceful degradation (commit 5bfe272).
- ~~**Admin Corpus Tab v2**~~ — **DONE (June 2026):** AdminModal restructured to 4 top-level tabs (Corpus, Feedback, Contributors, Notes Queue). Corpus has 3 sub-views: Documents (paginated list, View Sheet, Remove AlertDialog + `removed_urls` blocklist write), Sources (read-only license, visibility toggle), Pipelines (stats + command reference). New components: `alert-dialog.tsx`, `CorpusDocumentsPanel.tsx`. New endpoint: `GET /admin/corpus/documents`.
- ~~**`documents.original_title` not captured**~~ — **DONE (Session A, June 2026):** All 5 ingest insert sites now capture `original_title`. Migration 053 backfilled 3,678 rows. Column is nullable text; surface only when it differs from `title`.
- ~~**`removed_urls` blocklist not wired**~~ — **DONE (June 2026):** Migration 054 applied. `DELETE /admin/document/{id}` upserts to `removed_urls` before deleting. `youtube_ingest.py` checks `removed_urls` before each ingest call (marks `status=blocked`, non-fatal).
