...
**6. Aliases + sentinel cleanup + strict mode** (T2·2) — add Deere/Brown/Bedford/Church Life Class aliases (forward-looking only — existing docs already resolve) — **OPEN**. Reassign the 3 sentinel docs: Kneeling Christian → An Unknown Christian (known) — **OPEN**. **"So Great a Salvation" + "The 59 One Another's": RESOLVED (Alex, 2026-07-11)** — author/source genuinely unknown, both kept on sentinel and set silent_context (never cited by name). Executed in migration 059; live DB verified both silent_context on 2026-07-11. Metadata (author/source_name) stays NULL BY DECISION, not omission — do NOT re-flag as "needs Alex." Delete the `murray_surrender.pdf` stray duplicate — **OPEN**. Strict mode in shared_ingest refuses silent-sentinel — **OPEN**. (Non-write steps may run under the interim gate; real write steps are blocked until #5.5 lands.) *Stop: two-doc sub-item resolved; silent-sentinel class NOT fully ended — aliases, Kneeling Christian reassignment, Murray duplicate deletion, and strict mode still open.*
