...
- **DONE (2026-08-08), first pass — not a complete solution.** Sub-chunk exclusion mechanism for embedded non-teacher text sharing a chunk with real teacher material (flagged 2026-08-06, full list in archive). Implemented in `backend/app/services/quote_subchunk_exclusion.py`, wired into `quote_verifier.py` as a new `subchunk_exclusion` rule; regression tests in `scripts/test_quote_verifier.py` (cases 14–19 — independently re-run 2026-08-08, 19/19 pass, not just trusted from the build report); read-only dry-run in `scripts/dry_run_subchunk_exclusion.py` (confirmed no write calls). Three patterns, each keyed on an explicit structural marker: translator footnotes signed "-- Translator"; block quotations introduced by `:—` or `writes:`/`write:`/`wrote:`; Heidelberg/directory catechism Q&A introduced by a named-document marker.

  **Verified against real chunks, not just asserted — three residual gaps, not two:** (a) unmarked inline third-party narrative — Andrew Murray's *With Christ in the School of Prayer*, chunks 231/233, carries paragraphs of third-person George Müller biography with no quotation marks or structural marker at all; undetectable by construction. (b) cross-chunk continuation — the catechism detector is single-chunk-scoped; checked directly against the corpus's one concrete instance (*The New Life*, chunks 181→182), where generous chunk overlap happens to re-trigger the marker in chunk 182, so this specific case does not currently fail — but a less generous overlap elsewhere in the corpus could still slip through undetected. (c) **false-exclusion — the opposite failure, undocumented until this check:** the `:—`/`writes:` detector fires on the structural marker alone regardless of who is being quoted (verified by reading `_RE_QUOTATION_INTRO`/`_RE_WRITES_BLOCK` directly) — a teacher's own writing that quotes Scripture or another source using that same typographic convention would be wrongly excluded. Risk is very low for the translator-footnote pattern (its "-- Translator" signature is not something a teacher would organically write) and low for the catechism pattern (gated on a specific named-document marker), but real and not negligible for `:—`/`writes:` specifically.
