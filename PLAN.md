...
- **DONE (2026-08-08), first pass + gap-closing pass.** Sub-chunk exclusion mechanism for embedded non-teacher text sharing a chunk with real teacher material (flagged 2026-08-06, full list in archive). Implemented in `backend/app/services/quote_subchunk_exclusion.py`, wired into `quote_verifier.py` as a new `subchunk_exclusion` rule; regression tests in `scripts/test_quote_verifier.py` (cases 14–25 — independently re-run 2026-08-08, 25/25 pass); read-only dry-run in `scripts/dry_run_subchunk_exclusion.py` (confirmed no write calls). Four patterns: translator footnotes signed "-- Translator"; block quotations introduced by `:—` or `writes:`/`write:`/`wrote:`; Heidelberg/directory catechism Q&A introduced by a named-document marker; and inline Müller quotations in the George Müller chapter (long single-quoted paragraphs or all-caps single-quoted phrases inside chunks that mention "Muller").

  **Verified against real chunks — residual gaps now reduced to two:** (a) cross-chunk catechism continuation — the detector is single-chunk-scoped; checked directly against the corpus's one concrete instance (*The New Life*, chunks 181→182), where generous chunk overlap happens to re-trigger the marker in chunk 182, so this specific case does not currently fail — but a less generous overlap elsewhere in the corpus could still slip through undetected. Not safely closable without cross-chunk context or a schema change. (b) **false-exclusion:** the `:—`/`writes:` detector fires on the structural marker alone regardless of who is being quoted — a teacher's own writing that quotes Scripture or another source using that same typographic convention would be wrongly excluded. Risk is very low for the translator-footnote pattern (its "-- Translator" signature is not something a teacher would organically write), low for the catechism pattern (gated on a specific named-document marker), and low for the Müller-inline pattern (scoped to chunks that mention "Muller"), but real and not negligible for `:—`/`writes:` specifically.
