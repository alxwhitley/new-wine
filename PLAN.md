...
- ❗ **No canonical full-document text exists.** `content_summary` is a ~600-char summary (619/3,796 populated). Full text lives only in chunks. Chunk `content` IS verbatim source text — the verifier's ground truth (and chunks are committed before any quote hook fires — no ordering problem, verified Jul 8).
