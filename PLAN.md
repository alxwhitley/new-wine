...
- **DONE (2026-08-08)** — Build a sub-chunk exclusion mechanism for embedded non-teacher text sharing a chunk with real teacher material (Müller quotations, translator footnotes, catechism inserts — flagged 2026-08-06, full list in archive). Implemented in `backend/app/services/quote_subchunk_exclusion.py`, wired into `quote_verifier.py` as a new `subchunk_exclusion` rule, regression tests in `scripts/test_quote_verifier.py` (cases 14–19), dry-run in `scripts/dry_run_subchunk_exclusion.py`. Covers translator footnotes, `:—`/`writes:` block quotations, and Heidelberg/directory catechism Q&A; documented residual risks for unmarked inline Müller quotations and cross-chunk catechism answer continuations.
