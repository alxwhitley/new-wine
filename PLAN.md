...
5. **Matcher DONE (2026-08-07)** — `match_stored_position()` in `backend/app/services/stored_position_topics.py` (phrase lists from OD #16 V1; debate wins; paper-fenced out; multi-match → None + log). Tests: `scripts/test_stored_position_topics.py` Tier A+B green (all six keys live `is_current`).
6. **Evidence-injection wiring DONE (2026-08-08, commit `eca8070`)** — `backend/app/services/stored_position_evidence.py` (new) + `producer.py`'s `produce()`: on a match, the position's underlying PROPOSITIONS (never rendered text) replace normal retrieval's chunk set, then run through the exact same generation/verification/citation pipeline unchanged — the accepted one-hop design (CLAUDE.md Settled decision #18), not the two-hop store-then-synthesize shape pressure-tested and rejected 2026-08-04. The live license/visibility gate and commentary exclusion are re-applied per evidence proposition at serve time, not trusted from build time — real gap, since `position_evidence` rows were gate-filtered only when the position was built, and a source's visibility can (and here, did) change since. A pre-build bug was caught and fixed during verification: the position lookup originally required `requested_teacher_id IS NULL`, which silently missed 4 of the 6 seeded positions (built via teacher-explicit asks, not topic-only ones) — corrected to look up by `topic_key` alone, fail-safe to `None` if that ever becomes ambiguous. **Verified end-to-end with real generation across all six seeded topics — zero stored-position-text leakage into any served answer.** Three topics (fasting, deliverance from demons and spiritual warfare, how to pray effectively) currently no-op to normal RAG: their sole evidence source, Vlad Savchuk, is presently unlicensed/hidden (Phase 1.3 above) — correct fail-safe behavior, not a bug, and will start injecting once he's unhidden. Three (the divine exchange at the cross, holiness and personal purity, can a believer lose their salvation) inject live today. The corpus-scope "can a believer lose their salvation" position visibly narrowed from its original 4 contributors (Savchuk, Prince, Ravenhill, Kreighbaum) to 2 (Prince, Kreighbaum) under the live gate — the served answer's picture of "what the corpus says" can now diverge from the stored position's own cached text purely as a function of Phase 1.3 visibility state, worth knowing if anyone reads a stored position's `content` column directly. Debate topics, paper-fenced topics (baptism/tongues), and unrelated questions confirmed to never route through this path. **Still open, out of this build's scope:** production concurrency/rollout under real load (this session verified single-request correctness only, per Alex's explicit instruction); whether draft-status positions may serve unreviewed was not re-litigated (matches Settled decision #1's standing "no human review gate on the serving path"); a pre-existing, already-documented `match_position_paper()` semantic over-match (see CLAUDE.md Landmines) can cause a stored-position match to defer to the paper-fence path instead of injecting — narrow (observed on 1 of 6 phrasings tested for one topic), not new, and the precedence (paper wins) is a deliberate, conservative choice made this session, not a bug.
