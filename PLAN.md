...
- `[∥ safe]` **Phase 1.3** (from the 2026-08-01 accuracy sequence): reverse hidden-by-default + inventory (Settled Decision #12, CLAUDE.md). **Inventory DONE 2026-08-07 (read-only, no flip)** — live counts: owned/shown 2, public_domain/shown 28, unlicensed/shown 29, unlicensed/hidden **14**. Schema `sources.visibility` DEFAULT still `'hidden'`; registration scripts still fail-closed to hidden. **DONE for 3 of 14 (2026-08-09) — Ravenhill, Savchuk, Poonen flipped `hidden`→`shown`.** Visibility-only DB change (`license_status`/`retrievable` untouched, both stay `unlicensed`/`false`; sentinel + the other 11 hidden rows untouched — exactly 3 rows updated, confirmed by rowcount). Pre-flip live re-check matched the 2026-08-09 census exactly (no drift). **Verified against the real serving path**, not just the DB row (`producer.produce()`, the same function the async worker runs): a plain-retrieval question ("why does revival tarry") returned a real answer citing 3 distinct Ravenhill documents with correct attribution (`verified_references` confirms `Leonard Ravenhill`); a deliverance/spiritual-warfare question independently confirmed via `match_stored_position()` to hit the `deliverance from demons and spiritual warfare` stored-position topic, and returned a real position-backed answer built entirely from Savchuk evidence (15/15 citations `Vlad Savchuk`) — this topic was a no-op before the flip (Phase 3 item 5). **Remaining 11 hidden rows are untouched by design**: sentinel "Unassigned — needs source" must never flip (Invariant 3); the other 10 are empty shells (0 documents), nothing to flip. Rollback (visibility-only, seconds): `UPDATE sources SET visibility='hidden' WHERE id IN (Ravenhill, Savchuk, Poonen's ids)`. **The policy is already settled** (CLAUDE.md Settled decision #12, 2026-08-01: hidden-by-default reverses; new material defaults visible; everything currently hidden becomes visible eventually — "buys time, not a pass").
...
- **Curation rescoped 2026-08-08 (Alex's decision — records-only; no pipeline, detector, or flagging code touched).** Quote extraction from ALL book-type source documents (`source_type='book'`, all 53 book documents — including all 10 Andrew Murray books and Doug Kreighbaum's 4 books) is **tabled indefinitely**. Reason: `docs/audits/book_structure_diagnostic.md` (read-only, run 2026-08-08) found no body/apparatus or chapter-boundary structure exists anywhere in the schema for books — a book is a flat chunk run; `quote_ineligible_reason` covers only 66 of 25,064 book chunks across 10 of 53 documents (all Andrew Murray); the one existing chapter detector (`detect_book_chapters()`) is unwired with two documented, unfixed accuracy regressions (CLAUDE.md's book-length-extraction Landmine; Open Decision #21). Whether/how to build proper boundary detection for books is now a separate future decision, explicitly not blocking anything else.
  ...
  **Rescoped target: Derek Prince plus all other currently-visible non-book teachers** (`source_type != 'book'` — sermon/transcript/paper/magazine material), expanding past the original 2-teacher seed toward the existing 50–100 first-pass approved-quote target (`docs/plan-archive.md`). Live-DB-confirmed 2026-08-08 non-book document counts by currently-**shown** teacher (excluding Precept Austin — permanently excluded from the quote pipeline regardless of book/non-book status — and excluding pure-commentary sources, out of scope for quotes under the standing commentary-exclusion rule): **Derek Prince 496** (491 sermon + 5 magazine_article — by far the deepest bench), New Wine Magazine 15, CLF Church 14, Daniel Kolenda 11, Jack Deere 6, Carter Conlon 6, **Doug Kreighbaum 5** (1 manual + 3 papers + 1 sermon — his 4 *books* stay tabled under the exclusion above), Charles Simpson 4, Bob Mumford 4, Michael Brown 2, Ruth Prince 2, Ern Baxter 2, Don Basham 2, Oswald J. Smith 1. **Corrected 2026-08-09** — Vlad Savchuk (126 non-book docs), Leonard Ravenhill (117), Zac Poonen (50) are no longer hidden (Phase 2's Phase 1.3 entry above), so curating their material is no longer wasted work; extraction for them hasn't started — Prince curation (below) remains the active thread, these three are now eligible next candidates, not yet scheduled.
  **DONE (2026-08-09) — Derek Prince non-book extraction + review, both complete.** Extraction: the logged run (`logs/extract_prince_20260809_080948.log`) inserted 249 verified candidates as `status='pending'` across 249 of 496 documents — **corrects the earlier "250" figure**: the DB's 250th pending row (`bc3f71fd…`) came from an untracked separate invocation ~65s before the logged run started, is out of this batch's scope, and was left untouched (flagged for Alex). Review, same session: all 249 independently re-verified fresh against live chunk content (249/249 passed), then fully read/scanned for two defect classes the automated verifier can't catch — 10 rejected (6 dominated by verbatim Scripture text, 4 incoherent dangling fragments; logged to `quote_verification_log`, left `pending`), 239 cleared (`document_quote_clearance` inserted) and approved. Live re-query: Prince `approved`=240 (239 new + 1 pre-existing). Full detail: rhemata-status.md. **Recommended, not decided:** harden the extractor with a majority-Scripture check and an unbalanced-quote-mark check before re-running against the remaining 247 docs.
  **Flagged, not fixed — `pending` vs `draft`.** Migration 086 added `quotes.status='pending'` for exactly what migration 082's original `draft` status already meant ("awaiting human review before serving") — `draft` was simply orphaned, not repurposed, when 2026-08-08 removed the human-approval step (`create_and_approve_quote()` never creates a `draft` row). The schema now carries two status values doing the same job; likely should have reused `draft` instead of adding `pending`. Worth consolidating in a future session — Alex's call, not actioned here. Full reasoning: rhemata-status.md.
...
- `[∥ safe]` **#7** — `documents.full_text` backfill for pre-chokepoint docs (chunks-only today; every doc ingested since #7 shipped already gets it). **Quantified 2026-08-09 (corpus census, read-only, Grok): 58/3,597 documents have `full_text`, 3,539 missing** — supersedes the 2026-08-07 "56 present" probe (not a fixed number — re-query before acting).
