...
Last verified: 2026-07-23 (Study Panel geometry v3: nested-in-card, Phases 0-4 — see directly below).
Records reconciled: 2026-07-23 (Chat-only beta: gate Study/Discover, Phases 0-5 — see the section below the new one).
...
- **BLOCKER — panel fails to open when the chat textarea has focus. BETA-BLOCKING.** Clicking a verse/teacher reference while the chat textarea is focused silently does nothing — no panel opens, no error. Traced with full event instrumentation: `pointerdown` → `mousedown` → `focusout` (textarea) fire, then **nothing** — no `mouseup`, no `click`, no `focusin` on the reference button. The click sequence is interrupted between mousedown and mouseup, most likely because the textarea's `onBlur` (`setInputFocused(false)` in `chat-input.tsx`) triggers a re-render the browser can't reconcile against the in-flight click. **Confirmed pre-existing via an isolated git worktree** at `a60cc22` (the commit immediately before this session began, still on the old floating-overlay geometry) — identical repro, identical failure signature, on code this session never touched. **Severity is elevated by the chat-only-beta change from the prior session (`893bf0b`, pushed):** the in-chat panel is now the *only* route to Study content at all — standalone `/study` has no nav entry point anymore — so this silently breaks the sole entry point on the single most common real user path: type a question, get an answer, click a reference in it. The textarea very plausibly still has focus at that exact moment from typing. Recommend this becomes the next session, ahead of SP5 (mobile bottom-sheet).
- **BLOCKER — focus restoration on panel close lands on `<body>`, not the trigger.** Closing the panel does not return focus to the reference that opened it. Traced: Radix's initial auto-focus-on-open lands on the panel's Pin button (first focusable descendant); `StudyPanel`'s own `previouslyFocusedRef.current = document.activeElement` effect (which is supposed to capture the *pre-open* focus target for later restoration) appears to run *after* Radix's auto-focus effect already moved focus, so it captures the Pin button instead of the original trigger. Once the panel finishes closing (~300ms later) and the Pin button is removed from the DOM, focus falls through to `<body>` — a keyboard user closing the panel loses their place entirely. **This cause is a hypothesis, not a conclusion — it has not been proven by writing and testing a fix**, only reasoned from React's effect-ordering model (child effects before parent effects) and the observed before/after focus targets. Reasoned, not re-verified against baseline, to be pre-existing (same reasoning as the Tab-loop finding above: this is React-tree/effect-timing behavior, not portal-location-dependent).
