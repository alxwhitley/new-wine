...
3. **`AGENTS.md`** changes only for a genuinely new invariant, landmine, or
   settled decision meant to outlive this session — never to narrate what
   happened. The eviction rule in AGENTS.md already governs that file; this
...
AGENTS.md's own eviction rule already exists to prevent, just never extended
past AGENTS.md itself. Cutting text under this contract never destroys the
