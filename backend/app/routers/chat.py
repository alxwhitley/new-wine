...
from app.services.position_papers import (
    match_position_paper, get_paper_body, render_paper_voice_with_disclaimer,
)
...
from app.services.answer_toolbox import (
    expand_query, hybrid_search_rrf, _is_citable, fetch_neighbor_chunks_batch,
    is_word_study_query, is_commentary_chunk, exclude_commentary_chunks,
    SOURCE_KIND_FUSION_WEIGHTS, INCLUDE_COPYRIGHTED_ENV, ANSWER_SYSTEM_BLOCKS,
    match_background_topics, _ensure_background_topics, _get_cohere,
    _extract_answer_from_raw, _NO_ANSWER_FALLBACK,
    _ATTRIBUTION_REFUSAL, _ungrounded_reference_teachers,
)
...
# injection below — now lives in answer_toolbox (moved 2026-08-07, mirror-
# unification batch 1). Accessed as answer_toolbox._PILLAR_BY_KEY below, not
# a `from`-import — see that module's REBOUND-GLOBAL warning. _PILLAR_BY_KEY
# itself is never rebound, but the sibling background-topic state
# (_background_topics et al.) IS, so this file keeps one consistent
# module-attribute access pattern for all answer_toolbox module-level state
# rather than from-importing some names and not others.
#
# _ensure_background_topics, _get_cohere, match_background_topics, and the
# other functions imported above ARE safe as `from`-imports: function
# objects themselves are never reassigned by the loaders, only the data
# they populate (_background_topics, _ai, _cohere_client,
# _alias_to_topic_key, _background_topics_loaded) is -- and this file never
# reads those five names directly except via answer_toolbox.* below.
...
# _NO_ANSWER_FALLBACK, _ATTRIBUTION_REFUSAL, and _extract_answer_from_raw moved
# to answer_toolbox.py (2026-08-07, mirror-unification batch 1) -- imported at
# the top of this file, not redefined here.
...
# _ungrounded_reference_teachers moved to answer_toolbox.py (2026-08-07,
# mirror-unification batch 1) -- imported at the top of this file.
