...
| `propositions.py` | Extraction + storage. v3 prompt default; `EXTRACTION_PROMPT_V4` exists but is unwired — requires `prompt_version="v4"` explicitly. `process_document()` takes optional `name_pattern`/`verse_lookup`/`vocab_matcher` to activate the closeness-check pre-write gate (default off, byte-identical when omitted). Also wires an ALWAYS-ON scripture-reference grounding strip (PLAN.md #45.5, no opt-out — see CLAUDE.md Invariant 11) inside `extract_propositions()` itself: every reference found in a proposition's `content` is checked via `reference_grounding.check_reference_grounded()` against the source text already in scope; UNGROUNDED/UNCERTAIN references have only their own boundary-safe span stripped (never a blind `str.replace`), everything else in `content` left verbatim, every strip logged to gitignored `reference_grounding_review/stripped_references.jsonl`. No proposition is ever dropped by this step |
| `reference_grounding.py` | Shared GROUNDED/UNGROUNDED/UNCERTAIN predicate (PLAN.md #45.5) — is a scripture reference actually present in given source text? `find_reference_spans()` locates every parseable "Book Chap:Verse[-Verse]" substring; `check_reference_grounded()`'s citation-string arm always runs (reuses `reference_verifier._parse_verse_or_range`, imported not forked); an optional verse-wording arm (only if `verse_lookup` supplied) reuses `closeness_check`'s `_anchor_extend_density_span` fuzzy matcher for translation-variance wording matches. Source-agnostic `source_text: str` signature — used unchanged by both `propositions.py` (prevention call site, no `verse_lookup` available, so only the citation-string arm ever fires there) and `detect_reference_fabrication.py` (detection, `verse_lookup` supplied, both arms fire). Whole-document scoping only — no chunk/paragraph backreference exists on `propositions`, so it cannot distinguish "this reference is genuinely tied to this claim" from "this reference exists somewhere else in a long document" (confirmed real via the Ravenhill Phil 4:8-9 case, PLAN.md #45.5 — an accepted, disclosed limitation, not a bug). Known defect, disclosed not fixed: `check_reference_grounded()` doesn't apply the same period-to-space normalization `find_reference_spans()` already does before its own parse call, so dotted abbreviations ("1 Cor.", "Matt.") always read UNCERTAIN instead of being evaluated — verified structurally to only ever push a reference toward UNCERTAIN, never mask a genuine UNGROUNDED |
...
| `citation_verifier_layers.py` | Three-layer, cheapest-first citation-grounding verifier (PLAN.md #45.6) — widens recognition beyond `reference_grounding.py`'s compact-form-only scan to spoken/free-form shapes. Layer 1: regex-only, reuses `find_reference_spans()` plus five widened patterns (book+chapter+verse in various orders/forms, including bare "chapter N:M" colon form with no literal "verse" word — fixed 2026-07-28, commit `ff74a42`); every candidate re-validated through `word_or_digit_to_int()` + `_parse_verse_or_range()`, no bypass. Layer 2: document-wide book scope (book named anywhere + same chapter:verse found anywhere via a book-less pattern) — deliberately loose, `multi_book_document`/`distinct_book_count` exposed on the result for that reason; carries the same "chapter N:M" gap Layer 1 had, left unfixed (flagged, not this fix's scope). Layer 3: LLM read (`propositions._get_groq()`/`EXTRACTION_MODEL`), off by default (`llm_enabled=False`), wiring-proven only — 100% mocked test suite, never run against real Groq output; cost-estimated but not run, `docs/audits/layer3_llm_cost_estimate_2026-07-28.md`. `BOOK_MAP` is imported directly from `app.constants` (confirmed 2026-07-28, commit `4d9b193` — same object, not a fork, so it already reads any future fix to that module). Repurposed as of 2026-07-28: primary job is the generation-time confirming step #45.5's reversed anti-fabrication filter needs, not retroactive corpus audit |
| `closeness_check.py` | Phase 2 wording gate (PLAN.md #45) — trigram containment + longest-run secondary signal + scripture/name/theology/vocab exemption. `classify()` returns PASS/QUOTE_CANDIDATE/HOLD_TOO_LITTLE. Constants provisional pre-#46. `build_vocab_matcher()` (Phase 6) compiles `scripts/data/common_religious_vocab.json`'s 1,210 corpus-derived common phrases into a reusable fuzzy matcher reusing `_find_quote_span`'s own anchor/gap/density algorithm and constants (via shared `_anchor_extend_density_span`); wired through `exempt_for_containment`/`exempt_for_run` in masking order scripture→vocab→names→theology (vocab must run before the word-level name/theology stoplists, or they can fragment a vocab phrase's own anchor words first) |
