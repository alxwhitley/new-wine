...
│   ├── shared_ingest.py       # Shared document-writer chokepoint (ingest_document()): resolve -> insert -> chunk -> embed -> propositions. Corrected 2026-07-17 (previously stale at "converted: ingest.py (1 of 5)", verified false by direct code read): Converted — ingest.py, ingest_magazine.py, ingest_preceptaustin.py, ingest_lexicon.py. Not yet converted — ingest_commentaries.py, ingest_helloao.py (helloao was omitted from the original five-script framing entirely; it has its own independent Supabase REST insert path — `supabase.table("documents").insert()` / `.table("chunks").insert()` — a real, unconverted write path, not merely undocumented). **Second correction, same day (2026-07-17):** these two unconverted scripts are NOT equivalent risk. `ingest_commentaries.py` reads a pre-built SQLite dump from a hardcoded `/tmp` path (ephemeral, almost certainly gone on any reboot since) and is hard-shaped to that one collection's schema with no scraping/generic-format capability — it cannot be pointed at any other commentary source. Converting it is very likely busywork on a script that can no longer run; treat it as a retire-or-rebuild decision for Alex, not a pending conversion. `ingest_helloao.py` is the opposite: fetches from a live API, is resume-safe by design, and genuinely blocks the 8 further HelloAO commentaries PLAN.md #27 lists — real, live blocking work. Also: the "chokepoint gates corpus growth" framing is scoped to whichever specific unconverted script a growth item would route through — it does NOT mean all corpus growth is frozen. The YouTube/Sermonindex path (`youtube_ingest.py` → `ingest_file()` → `shared_ingest`) is fully converted and was never blocked by either #10 or #13.
...
- **Citable requires a real attributable name (Alex, 2026-07-12):** a document is `citation_mode = 'citable'` ONLY if a real, attributable name can be attached to it as its source or author. Anonymous or pseudonymous sources stay `silent_context`, always — even once assigned a real, named, servable `sources` row. Established by "The Kneeling Christian" (#6, 2026-07-12): reassigned from the sentinel to the real "An Unknown Christian" source (`public_domain`/`shown`), but `citation_mode` was deliberately held at `silent_context` — "An Unknown Christian" is a pseudonym, the explicit absence of a name, not an attributable identity, so the document is not citable despite now having a real, known, servable source. Do NOT read this document's `silent_context` as a leftover sentinel artifact in a future session and flip it — it is the correct, permanent state under this rule. **Adjacent, deliberately not acted on this session:** applying this rule strictly may mean other already-`citable` documents that trace to anonymous or pseudonymous sources are now out of compliance and need an audit. Flagged only, not investigated.
...
| `scripts/shared_ingest.py` | Shared document-writer chokepoint. `ingest_document()`: dedup (`documents.url`+`source_name` exact match, or filename-suffix match against `documents.file_path` when no URL — no `source_hash` column, never has existed) -> resolve attribution (`source_id` override or `resolve_from=(source_name, author)`) -> insert document row -> chunk (`chunk_fn`, default the shared token chunker) -> embed + insert chunks (**corrected 2026-07-17, verified false by direct code read: no `insert_mode` parameter exists at all** — one unconditional batched insert via a single `execute_values(...)` call; the earlier-described `"rest_per_chunk"`/`"psycopg2_batch"` mode split was never built as two selectable modes) -> propositions (`propositions_conn` to reuse caller's connection, else opens/closes its own). Also exposes `find_existing_fn`/`on_existing` (skip/reuse/delete_and_reingest) for the reuse patterns `ingest_preceptaustin.py` and `ingest_lexicon.py` use now that both are converted (**corrected 2026-07-17** — previously described as still pending); `ingest_commentaries.py` remains unconverted, but conversion is likely moot — its source SQLite dump lives at a hardcoded, ephemeral `/tmp` path and is almost certainly gone (verified 2026-07-17); see the retire-or-rebuild note on the Directory Structure entry above. Imports `embed_text`/`embed_batch` from `backend/app/services/embeddings.py` (fixes a latent bug: `ingest.py`'s old local `embed_text()` omitted `dimensions=1536`). |
...
| `scripts/ingest_commentaries.py` | Ingest HistoricalChristianFaith commentaries from SQLite DB. **Effectively dead as of 2026-07-17** — source is a hardcoded `/tmp` SQLite dump, almost certainly gone; script has no scraping/generic-format capability and can't target any other collection. Retire-or-rebuild decision for Alex, not a pending conversion (see PLAN.md #10 / Open Decisions #12). |
...
| `scripts/ingest_helloao.py` | Ingest Matthew Henry / Adam Clarke / JFB commentaries from the HelloAO Bible API — one document per book per commentary, REST-based, resolves source_id + wired to propositions. Flags: `--test`, `--book`, `--time-limit`. **Not yet converted to shared_ingest (confirmed 2026-07-17) — real, live blocking gap** (fetches from a live API, resume-safe by design), but scoped only to the 8 further HelloAO commentaries PLAN.md #27 lists as future work, not to corpus growth generally. |
...
**Corrected 2026-07-17 (kept above for provenance, not deleted):** direct
code read confirms `ingest_magazine.py`, `ingest_preceptaustin.py`, and
`ingest_lexicon.py` are now also converted and routed through
`shared_ingest.ingest_document()` — propositions can no longer be skipped
by omission for those three either. Only `ingest_commentaries.py` remains
NOT converted (own raw psycopg2 INSERTs, propositions called directly, not
via the shared writer). `ingest_helloao.py` — not named in this section's
original four-script list at all — is also NOT converted (own Supabase
REST `.insert()` calls) and the per-script-discipline gap applies to it
too.

**Second correction, same day (2026-07-17):** `ingest_commentaries.py`'s
source data (a hardcoded `/tmp` SQLite dump) is almost certainly gone —
this script very likely can't be run at all today, converted or not, so
its propositions gap is probably moot rather than live. `ingest_helloao.py`
is the real, live gap: it fetches from a live API and could run today.
Neither gap affects any other script — the per-script-discipline rule
above was never a global freeze.
