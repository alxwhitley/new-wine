...
- Tables: `documents`, `chunks`, `propositions`, `verses`, `saved_words`, `excerpts`, `guest_sessions`, `conversations`, `messages`, `interlinear_words`, `book_quotes`, `user_usage`, `sources`, `source_aliases`, `source_license_audit`, `app_settings`
...
- Step 6 — Corpus governance UI (June 2026): `/admin` Corpus section is now a two-tab view: Governance (default) and Pipelines. Governance: global safe_mode switch + per-source table (name, doc count, license_status dropdown, retrievable/visibility toggle), amber "unlicensed + shown" counter. License/visibility write immediately (no confirm dialog). Sentinel row `267a09ac…` renders locked — no controls, and backend rejects edits at 403. Pipelines: prior ingestion-progress cards relocated unchanged. `GET /admin/stats` (service-key, bypasses RLS) replaced broken anon-client HEAD count queries that returned 0. `GET /admin/license-sources` doc counts moved from per-source N+1 COUNT queries to a single bulk fetch aggregated in Python (~4.6s/timeout in production before; ~0.5s after).
