...
# ── Fixed targets (no credentials here — those are read inside run_checks) ────
...
def run_checks() -> int:
    """Everything below this line used to run at module import. It is
    byte-for-byte the same sequence, just no longer reachable by an import."""
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="Metering verification against the PRODUCTION database and API.",
    )
    parser.add_argument(
        "--apply",
        action="store_true",
        help="required acknowledgement for the production database writes this performs",
    )
